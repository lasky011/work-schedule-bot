...
## Что выгоднее (оплата в рублях, сентябрь 2026)

Нужны оба бота 24/7 и Mini App. Один процесс на проде ел ~250 МБ, два — около 400–500 МБ. Бесплатного always-on нет.

| Место | Цена | Регион / Telegram | Комментарий |
|---|---|---|---|
| **Amvera «Начальный»** | **290 ₽/мес** | Варшава или Москва с прокси Telegram | Самый дешёвый «под ключ». 500 МБ — впритык, если OOM → апгрейд до 490 ₽ |
| **4VPS EU, 1 ГБ** (напр. Австрия) | **~420 ₽/мес** | Европа, Telegram ок | Самый дешёвый 1 ГБ. Сами Docker + HTTPS (Caddy) |
| **Amvera «Начальный Плюс»** | **490 ₽/мес** | Варшава, SSL сам | 1 ГБ, git-деплой, без админки сервера |
| Timeweb Apps Backend 1/1 | ~510 ₽/мес | AMS-1 / FRA-1 | Удобно, но дороже Amvera |
| Timeweb VPS NL-15 | 639 ₽/мес | Амстердам | Ещё дороже за те же 1 ГБ |

**Выгодный старт:** Amvera «Начальный» 290 ₽, регион Warsaw. Не хватит памяти — в панели поднять до «Начальный Плюс» 490 ₽. Timeweb не брать ради экономии: он не дешевле.
...
5. Сборка подхватит `amvera.yaml`: точка входа `run_both.py` (оба бота), Mini App на 8080.
...
2. Если Compose в панели нет — тип **Dockerfile**, команда запуска `python3 run_both.py`.
